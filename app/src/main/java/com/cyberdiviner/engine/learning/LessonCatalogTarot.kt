            ...
            questions = listOf(
                QuizQuestion(
                    ...
                    items = listOf(
                        MatchItem("愚者", "新开始与未知"),
                        MatchItem("死神", "结束与转化"),
                        MatchItem("女祭司", "直觉与内在智慧"),
                        MatchItem("战车", "意志力与前进")
                    ),
                    explanation = "愚者象征旅程的起点与可能性；死神代表旧阶段的终结与转化；女祭司掌管内在知识与直觉；战车则对应意志驱动的行动。"
                ),
                QuizQuestion(
                    id = "C1_Q2",
                    ...
                    correctIndex = 1,
                    explanation = "大阿卡纳描绘的是人生原型阶段的象征性旅程，从无知到整合，而非运势预测或神灵信仰。"
                )
            ),
        ...
        Lesson(
            ...
            questions = listOf(
                QuizQuestion(
                    id = "C2_Q1",
                    type = QuizType.MATCHING,
                    prompt = "将下列情境与最接近的元素配对（火＝行动、水＝情感、风＝思维、土＝物质）",
                    items = listOf(
                        MatchItem("决定是否跳槽到新公司", "风/思维"),
                        MatchItem("和伴侣讨论未来的方向", "水/情感"),
                        MatchItem("制定下季度的储蓄计划", "土/物质"),
                        MatchItem("开始一个新的健身习惯", "火/行动")
                    ),
                    explanation = "跳槽是思维决策（宝剑/风）；伴侣对话关乎情感连结（圣杯/水）；储蓄计划是物质规划（星币/土）；新习惯需要行动驱力（权杖/火）。"
                ),
                QuizQuestion(
                    id = "C2_Q2",
                    type = QuizType.MATCHING,
                    ...
                    explanation = "权杖对应火（行动）、圣杯对应水（情感）、宝剑对应风（思维）、星币对应土（物质），这是塔罗四元素的基础对应体系。"
                )
            ),
            unlockReward = UnlockReward(
                UnlockType.TERM_ANNOTATION,
        ...
        Lesson(
            ...
            questions = listOf(
                QuizQuestion(
                    ...
                    options = listOf(
                        "逆位代表厄运和不幸",
                        "逆位意味着这张牌的能量出现干扰，需要多角度判断",
                        "逆位应该忽略，只看正位含义",
                        "逆位永远比正位的含义弱"
                    ),
                    correctIndex = 1,
                    explanation = "逆位表示该牌能量的运作方式发生了变化——可能是阻滞、内化、过度或延迟——而非简单的「好」或「坏」。需要结合情境具体分析。"
                ),
                QuizQuestion(
                    id = "C3_Q2",
                    ...
                    options = listOf(
                        "他会失去工作",
                        "他的前进动力可能受阻，或方向不够明确，需要先厘清目标",
                        "他不适合这份工作",
                        "这张牌没有意义"
                    ),
                    correctIndex = 1,
                    explanation = "逆位战车在工作议题中，通常暗示前进的能量受到干扰——可能是目标不够清晰、动力不足或外在阻力较大。这是一个调整方向的信号，而非灾难性的结论。"
                )
            ),
            unlockReward = UnlockReward(
                UnlockType.TERM_ANNOTATION,
        ...
        Lesson(
            ...
            questions = listOf(
                QuizQuestion(
                    id = "C4_Q1",
                    type = QuizType.MATCHING,
                    prompt = "将单牌解读的四个步骤按正确顺序配对",
                    items = listOf(
                        MatchItem("第一步", "明确问题"),
                        MatchItem("第二步", "识别牌面"),
                        MatchItem("第三步", "提取关键词"),
                        MatchItem("第四步", "给出建议方向")
                    ),
                    explanation = "先有清晰的问题，才能有效解读牌面；识别牌面后提取关键词；最终将关键词与问题结合，形成建议方向。这是从「输入」到「输出」的逻辑链。"
                ),
                QuizQuestion(
                    id = "C4_Q2",
                    type = QuizType.SINGLE_CHOICE,
                    ...
                    correctIndex = 1,
                    explanation = "有效的提问应该是具体的、开放的、与自身行动相关的。「我需要注意什么」将焦点放在可观察和可行动的层面。"
                )
            ),
            unlockReward = UnlockReward(
                UnlockType.TERM_ANNOTATION,
        ...
        Lesson(
            ...
            questions = listOf(
                QuizQuestion(
                    ...
                    options = listOf(
                        "她过去一直抓着稳定不放，现在内心的冲突正在消退，未来将展现出行动力和自信",
                        "她过去很穷，现在很痛苦，未来会变好",
                        "三张牌各说各的，没有关联",
                        "这个牌阵显示她不适合工作"
                    ),
                    correctIndex = 0,
                    explanation = "星币四正位暗示过去倾向保守固守；宝剑五逆位表示内心的纷争或纠结正在缓解；权杖女王正位指向未来充满热情与领导力的展现。三张牌串联出一个「从固守到释放到行动」的成长叙事。"
                ),
                QuizQuestion(
                    id = "C5_Q2",
                    type = QuizType.MATCHING,
                    prompt = "解读三牌牌阵时，按最有效的阅读顺序配对",
                    items = listOf(
                        MatchItem("第一步", "先看中间（现在）"),
                        MatchItem("第二步", "回溯左边（过去）"),
                        MatchItem("第三步", "展望右边（未来）")
                    ),
                    explanation = "以「现在」为锚点，先理解当前处境，再追溯原因，最后看发展趋势。这样叙事更连贯，避免三张牌变成割裂的碎片。"
                )
            ),
            unlockReward = UnlockReward(
                UnlockType.TERM_ANNOTATION,
        ...
        Lesson(
            ...
            questions = listOf(
                QuizQuestion(
                    ...
                    options = listOf(
                        "逐张翻译全部10张牌的含义，再加总",
                        "先聚焦第1张和第2张（主轴），理解核心矛盾，再逐步扩展",
                        "只看最后一张，因为它代表结果",
                        "跳过这个牌阵，永远不学"
                    ),
                    correctIndex = 1,
                    explanation = "高阶牌阵的关键是「抓主轴」——先理解现状与挑战的核心矛盾，其他位置是补充信息。逐张翻译会失去整体叙事。"
                ),
                QuizQuestion(
                    id = "C6_Q2",
                    type = QuizType.CASE_JUDGE,
                    ...
                    options = listOf(
                        "一切顺利，没有问题",
                        "当前处于不确定或模糊的状态（月亮），而挑战在于某个困境正走向结束但仍有余波（宝剑十逆位）",
                        "会有可怕的事情发生",
                        "这两张牌不能一起出现"
                    ),
                    correctIndex = 1,
                    explanation = "月亮正位常提示不确定、模糊或需要面对潜意识的时期；宝剑十逆位表示一段艰难的阶段正在过去，但残留影响仍在。主轴暗示：在迷雾中，旧的痛苦正逐渐消退，需要耐心等待清明。"
                ),
                QuizQuestion(
                    id = "C6_Q3",
                    type = QuizType.SINGLE_CHOICE,
                    ...
                    correctIndex = 1,
                    explanation = "凯尔特十字中，第1张代表当前处境，第2张横跨其上，代表当前面临的主要挑战或张力，两者共同构成牌阵的核心解读轴。"
                )
            ),
            unlockReward = UnlockReward(
                UnlockType.TERM_ANNOTATION,
