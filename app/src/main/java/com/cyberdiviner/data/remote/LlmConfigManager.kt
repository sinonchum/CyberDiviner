...
class LlmConfigManager(private val context: Context) {
    ...
    val provider: Flow<String> = context.dataStore.data.map { it[KEY_PROVIDER] ?: LlmProvider.OPENAI.name }
    val modelId: Flow<String> = context.dataStore.data.map { it[KEY_MODEL_ID] ?: "gpt-4o-mini" }
