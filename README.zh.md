...
CyberDiviner 是一款 Android 端占卜应用，把周易六爻、塔罗牌阵、面相解读、黄历宜忌、离线推理、电子颂钵和本地命簿整合到一个克制、干净、带朱砂红点缀的黑白界面中。
...
- **周易六爻**：摇动手机起卦，自动生成本卦、变卦和动爻解读，并提供结构化兜底。
- **赛博塔罗**：78 张塔罗牌，多种牌阵，翻牌动效与震感反馈。
- **视界摸骨**：相机引导入框，端侧提取面部特征，生成古风面相批语。
- **赛博黄历**：每日宜忌、能量值、幸运色与桌面小组件。
- **电子颂钵**：颂钵与木槌动画，配合本地音效形成轻量仪式交互。
- **因果命簿**：本地保存签文、卦象、塔罗和面相记录，并支持生成紧凑分享卡片。
...
离线路径通过 LiteRT-LM bridge 调用 CyberDiviner 微调的 Gemma 3 1B 端侧模型。用户可以在 Config 页面导入本地 `.task` 模型文件。每个功能都有输出规整层，用来限制格式漂移、提示词回声、英文混入和重复文本。基础规则引擎仍保留，用于非高级路径和兜底场景。
...
固定输出「载入签文」「逻辑解析」「最终断语」，适合快速问事。签文会规整为两行格式，保持在线与离线输出一致。
...
本地保存历史记录，并让塔罗、六爻、面相卡片遵循一致的标题与摘要逻辑。展开记录后可生成紧凑图片卡片用于分享。

**电子颂钵**

提供安静的重复点击仪式。颂钵与木槌动画配合本地音效，用于轻量放松和每日留存。
...
| AI | 在线模型适配、LiteRT-LM 本地推理、功能级输出规整、本地规则引擎 |
| 小组件 | Jetpack Glance |
...
在应用内 Config 页面选择推理模式、配置在线模型、启用本地模型，并导入或管理本地 `.task` 模型文件。应用会尽量保持不同模型之间的输出结构一致，让在线和离线结果在界面上呈现同一套产品规格。
